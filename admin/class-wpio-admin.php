...
class WPIO_Admin {
    ...
    public function add_plugin_admin_menu() {
        ...
        add_submenu_page(
            'upload.php',
            __('Optimization Logs', 'weirdopress-image-optimizer'),
            __('Optimization Logs', 'weirdopress-image-optimizer'),
            'manage_options',
            'wpio-logs',
    ...
    public function register_settings() {
        ...
        add_settings_field(
            ...
            __('Delete data on uninstalling', 'weirdopress-image-optimizer'),
    ...
    public function binary_section_callback() {
        ...
        if (!function_exists('exec')) {
            ...
            echo __('The exec() function is disabled on your server. This plugin will use PHP-only optimization methods, which may not be as efficient as using external binaries. See the Environment Information panel below for more details.', 'weirdopress-image-optimizer');
            echo '</p>';
            echo '</div>';
        }
    }
                ...
                <div class="wpio-sidebar">
                    ...
                    <div class="wpio-box">
                        ...
                        <ul>
                            ...
                            <li><?php _e('Works on shared hosting', 'weirdopress-image-optimizer'); ?></li>
                        </ul>
                    </div>
    ...
    private function render_environment_panel($capabilities) {
        ...
        <div class="wpio-environment-panel">
            <h2><?php _e('Environment Information', 'weirdopress-image-optimizer'); ?></h2>
            
            <div class="wpio-env-description">
                <p>
                    <?php _e('This panel shows your server\'s capabilities for image optimization. WeirdoPress Image Optimizer adapts to your server environment to provide the best possible results.', 'weirdopress-image-optimizer'); ?>
                </p>
            </div>
            ...
            <table class="widefat wpio-env-table">
                <thead>
                    <tr>
                        <th><?php _e('Feature', 'weirdopress-image-optimizer'); ?></th>
                        <th><?php _e('Status', 'weirdopress-image-optimizer'); ?></th>
                        <th><?php _e('Impact', 'weirdopress-image-optimizer'); ?></th>
                    </tr>
                </thead>
                <tbody>
                    <tr>
                        <td><?php _e('PHP Version', 'weirdopress-image-optimizer'); ?></td>
                    ...
                    <tr>
                        <td><?php _e('exec() Function', 'weirdopress-image-optimizer'); ?></td>
                        <td>
                            <?php 
                    ...
                    <tr>
                        <td><?php _e('GD Library', 'weirdopress-image-optimizer'); ?></td>
                        <td>
                            <?php 
                    ...
                    <tr>
                        <td><?php _e('ImageMagick', 'weirdopress-image-optimizer'); ?></td>
                        <td>
                            <?php 
                    ...
                    <tr>
                        <td><?php _e('WebP Support', 'weirdopress-image-optimizer'); ?></td>
                        <td>
                            <?php 
                    ...
                    <tr>
                        <td><?php _e('AVIF Support', 'weirdopress-image-optimizer'); ?></td>
                        <td>
                            <?php 
                    ...
                    <tr>
                        <td><?php _e('Memory Limit', 'weirdopress-image-optimizer'); ?></td>
            ...
            <div class="wpio-env-recommendations">
                <h3><?php _e('Recommendations', 'weirdopress-image-optimizer'); ?></h3>
                
                <?php if (!$capabilities['exec_enabled']): ?>
                    <div class="wpio-recommendation">
                        <p>
                            <strong><?php _e('Enable exec() function:', 'weirdopress-image-optimizer'); ?></strong>
                            <?php _e('For better compression results, ask your hosting provider to enable the PHP exec() function. With exec() enabled, the plugin can use specialized binaries like cwebp, avifenc, and jpegoptim for higher quality compression.', 'weirdopress-image-optimizer'); ?>
                        </p>
                    </div>
                <?php endif; ?>
                ...
                <?php if (!$capabilities['imagick_enabled']): ?>
                    <div class="wpio-recommendation">
                        <p>
                            <strong><?php _e('Install ImageMagick extension:', 'weirdopress-image-optimizer'); ?></strong>
                            <?php _e('The ImageMagick PHP extension provides better image processing capabilities than GD. Ask your hosting provider if they can enable it for your account.', 'weirdopress-image-optimizer'); ?>
                        </p>
                    </div>
                <?php endif; ?>
                ...
                <?php if (!$capabilities['webp_support']): ?>
                    <div class="wpio-recommendation">
                        <p>
                            <strong><?php _e('Enable WebP support:', 'weirdopress-image-optimizer'); ?></strong>
                            <?php _e('WebP images are smaller than JPEGs and PNGs while maintaining similar quality. Consider upgrading your PHP version or asking your host to add WebP support to your GD or ImageMagick installation.', 'weirdopress-image-optimizer'); ?>
                        </p>
                    </div>
                <?php endif; ?>
                ...
                <?php if ($this->parse_memory_size($capabilities['memory_limit']) < 128 * 1024 * 1024): ?>
                    <div class="wpio-recommendation">
                        <p>
                            <strong><?php _e('Increase memory limit:', 'weirdopress-image-optimizer'); ?></strong>
                            <?php _e('A higher PHP memory limit allows for processing larger images. Consider increasing it to at least 128M if you work with large images.', 'weirdopress-image-optimizer'); ?>
                        </p>
                    </div>
                <?php endif; ?>
                ...
                <?php if ($capabilities['exec_enabled'] && $capabilities['imagick_enabled'] && $capabilities['webp_support'] && $this->parse_memory_size($capabilities['memory_limit']) >= 128 * 1024 * 1024): ?>
                    <div class="wpio-recommendation wpio-recommendation-good">
                        <p>
                            <strong><?php _e('Your environment is well-configured:', 'weirdopress-image-optimizer'); ?></strong>
                            <?php _e('All necessary features are available for optimal image compression. You should get excellent results with this plugin.', 'weirdopress-image-optimizer'); ?>
                        </p>
                    </div>
                <?php endif; ?>
            </div>
    ...
    public function add_media_columns($columns) {
        $columns['wpio_status'] = __('Optimization', 'weirdopress-image-optimizer');
        return $columns;
    }
    ...
    public function display_media_column_content($column_name, $attachment_id) {
        ...
        if (!wp_attachment_is_image($attachment_id)) {
            echo '<span class="wpio-badge wpio-badge-not-image">Not an image</span>';
            return;
        }
        ...
        if (!$file_path || !file_exists($file_path)) {
            echo '<span class="wpio-badge wpio-badge-error">File not found</span>';
            return;
        }
        ...
        if ($optimization_data) {
            ...
            echo '<span class="wpio-status-text">Optimized</span>';
            echo '</div>';
            ...
            echo '<div class="wpio-status-row">';
            echo '<span class="wpio-status-icon wpio-status-optimized"></span>';
            echo '<span class="wpio-status-text">' . strtoupper($extension) . ' format</span>';
            echo '</div>';
        } else {
            // Not optimized yet
            echo '<div class="wpio-status-row">';
            echo '<span class="wpio-status-icon wpio-status-not-optimized"></span>';
            echo '<span class="wpio-status-text">Not optimized</span>';
            echo '</div>';
            echo '<button class="button button-small wpio-optimize-single" data-id="' . esc_attr($attachment_id) . '">Optimize</button>';
        }
        ...
        if ($has_webp || $has_avif) {
            ...
            if ($has_webp) {
                echo '<span class="wpio-format-badge wpio-webp-badge" title="WebP: ' . size_format($webp_size, 1) . ' (' . $webp_savings . '% savings)">WebP</span>';
            }
            
            if ($has_avif) {
                echo '<span class="wpio-format-badge wpio-avif-badge" title="AVIF: ' . size_format($avif_size, 1) . ' (' . $avif_savings . '% savings)">AVIF</span>';
            }
    ...
    public function add_attachment_fields($form_fields, $post) {
        ...
        if (!$optimized) {
            $form_fields['wpio_status'] = array(
                'label' => __('Optimization', 'weirdopress-image-optimizer'),
                'input' => 'html',
                'html'  => '<span class="wpio-status-warning">' . __('Not optimized', 'weirdopress-image-optimizer') . '</span>',
            );
            return $form_fields;
        }
        ...
        if ($saved_percent > 0) {
            $html .= '<span class="wpio-status-ok">' . sprintf(__('Saved %s%%', 'weirdopress-image-optimizer'), number_format($saved_percent, 1)) . '</span><br>';
            $html .= '<strong>' . __('Original size', 'weirdopress-image-optimizer') . ':</strong> ' . $this->format_bytes($original_size) . '<br>';
            $html .= '<strong>' . __('Optimized size', 'weirdopress-image-optimizer') . ':</strong> ' . $this->format_bytes($optimized_size) . '<br>';
            $html .= '<strong>' . __('Saved', 'weirdopress-image-optimizer') . ':</strong> ' . $this->format_bytes($saved_bytes);
        } else {
            $html .= '<span class="wpio-status-ok">' . __('Already optimized', 'weirdopress-image-optimizer') . '</span>';
        }
        ...
        if (file_exists($webp_path)) {
            $html .= '<br><strong>' . __('WebP Version', 'weirdopress-image-optimizer') . ':</strong> ' . __('Yes', 'weirdopress-image-optimizer');
        }
        
        if (file_exists($avif_path)) {
            $html .= '<br><strong>' . __('AVIF Version', 'weirdopress-image-optimizer') . ':</strong> ' . __('Yes', 'weirdopress-image-optimizer');
        }
    ...
    public function display_logs_page() {
        ...
        echo '<h1>Image Optimization Logs</h1>';
        
        if (empty($logs)) {
            echo '<div class="wpio-empty-logs">';
            echo '<p>No optimization logs found. Optimize some images first!</p>';
            echo '</div>';
        } else {
            ...
            echo '<th>Optimized Size</th>';
            echo '<th>Savings</th>';
            echo '<th>WebP/AVIF</th>';
            echo '<th>Date</th>';
            echo '</tr>';
            echo '</thead>';
            echo '<tbody>';
    ...
    public function ajax_optimize_single_image() {
        // Check nonce
        if ( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( $_POST['nonce'], 'wpio_nonce' ) ) {
